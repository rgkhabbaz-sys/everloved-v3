...
const VoiceSession: React.FC<VoiceSessionProps> = ({ onEndSession, onSpeakingStateChange, activeProfile }) => {
    ...
    // --- NATIVE SPEECH HANDLING ---
    // Replaces external VAD library to avoid 404/Asset issues.

    useEffect(() => {
        // Initialize SpeechRecognition
        if (!window.webkitSpeechRecognition && !window.SpeechRecognition) {
            setError("Browser not supported (Chrome/Edge required).");
            return;
        }
        ...
        recognition.onresult = (event: any) => {
            ...
            if (isFinal && text.trim().length > 0) {
                // Determine if this is a complete thought (simple heuristic)
                // In native mode, we blindly trust 'isFinal' from the browser
                addLog(`Native Final: "${text}"`);
        ...
        recognition.onerror = (event: any) => {
            ...
            addLog(`Native Error: ${event.error}`);
        ...
        recognition.onend = () => {
            // If we should be listening but stopped (e.g. silence timeout), restart
            if (status === 'listening' && !isThinking) {
                try {
                    recognition.start();
                } catch (e) { /* ignore */ }
            }
